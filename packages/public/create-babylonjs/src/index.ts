...
import { generateGameSceneCode } from "./generators/gameSceneCode";
...
export type ProjectTemplate = "viewer" | "game";
...
async function main(): Promise<void> {
    ...
    const response = await prompts(
        [
            ...
            },
            {
                ...
                choices: [
                    { title: "Viewer / Scene — load glTF model with environment", value: "viewer" },
                    { title: "Commercial Game — arcade prototype with score loop + GUI", value: "game" },
                ],
            },
            {
                type: "select",
            ...
            },
            {
                ...
                choices: (_, values) => {
                    ...
                    if (values.moduleFormat === "umd" && values.projectTemplate !== "game") {
    ...
    const options: ProjectOptions = {
        ...
        projectTemplate: response.projectTemplate ?? "viewer",
        moduleFormat: response.moduleFormat,
        language: response.language,
        bundler: response.bundler === "none" && response.projectTemplate === "game" ? "vite" : response.bundler,
    };
...
function scaffoldProject(targetDir: string, options: ProjectOptions): void {
    ...
    const sceneCode = options.projectTemplate === "game" ? generateGameSceneCode(options) : generateSceneCode(options);
    writeFile(targetDir, `src/index.${ext}`, sceneCode);

    if (options.projectTemplate === "game") {
        writeFile(
            targetDir,
            "GAME_README.md",
            `# ${options.projectName}

Commercial game starter generated by create-babylonjs.

## Controls
WASD / Arrow keys — fly and collect orbs

## Next steps
1. Record gameplay for your crowdfunding trailer
2. Open Babylon Game Studio in bjs-monorepo for PM dashboard
3. Deploy demo to itch.io / Steam Coming Soon
